...
/** 搜索 topic:dsh-plugin 仓库（按最近更新排序，最新在前；分页直到 maxRepos 或结果耗尽）。 */
async function searchTopicRepos(token, maxRepos) {
  const repos = []
  for (let page = 1; repos.length < maxRepos && page <= 10; page++) {
    const res = await netFetch(`${SEARCH_URL}&page=${page}`, {
      timeout: 30000,
      headers: {
...
function itemFromRepo(r, validated, meta) {
  ...
  const item = {
    ...
    author: owner,
    stats: { subscribers: 0, rating: 0, rating_count: 0, views: 0, stars: r.stargazers_count || 0, forks: r.forks_count || 0 },
 ...
 */
export async function syncItems({ token = '', maxRepos = 200, cache = {} } = {}) {
  const repos = await searchTopicRepos(token, maxRepos)
  const items = []
  const nextCache = {}
  let validated = 0
  let fromCache = 0
